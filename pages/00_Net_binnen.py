...
st.set_page_config(page_title="Net binnen • KbM Nieuws", page_icon="🗞️", layout="wide")
inject_css(st)
...
st.title("Net binnen")
st.caption("Maximaal 4 uur geleden (realtime-ish).")
...
items, _ = collect_items(CATEGORY_FEEDS.get("Net binnen", []), query=query or None, max_per_feed=60, force_fetch=force_fetch)
items = [x for x in items if within_hours(x.get('dt'), 4)]
