...
FEEDS = {
    ...
    # RTL (via Google News RSS wrappers per sectie)
...
def resolve_google_news_url(gn_url: str) -> str:
    """Resolve Google News RSS wrapper to original URL (best-effort, fast)."""
    try:
        ...
        html = r.text or ""
        # sometimes the destination is in a meta refresh/canonical
        ...
        # final fallback: follow redirects; if we ended up elsewhere, use that
...
def fetch_readable_text(url: str) -> tuple[str, str]:
    """Return (title, text). Works for open sites like rtvmiddenholland + rtl, best-effort."""
    try:
        ...
        # Prefer <article>, then common WP containers (RTV MH is WP-like)
        containers = []
        containers += soup.select("article")
        containers += soup.select(".entry-content, .post-content, .content, .article__body, .article-content, main")
        if not containers:
