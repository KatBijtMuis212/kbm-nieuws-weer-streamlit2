...
def fetch_feed(feed_url: str):
    """Fetch and parse RSS/Atom with a requests UA (needed for some feeds like Google News)."""
    now = time.time()
    cached = _FEED_CACHE.get(feed_url)
    if cached and now - cached["t"] < _FEED_CACHE_TTL:
        return cached["d"]
    try:
        r = _SESSION.get(feed_url, timeout=_TIMEOUT, allow_redirects=True)
        r.raise_for_status()
        d = feedparser.parse(r.content)
    except Exception:
        # fallback to feedparser's own fetch (might still work for some feeds)
